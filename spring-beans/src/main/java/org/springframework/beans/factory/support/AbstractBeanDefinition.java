...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * <p>
	 * 将父类的BeanDefinition重写为自类的 将自类的BeanDefinition添加到父类中去
	 * <ul>
	 ...
	 * <p>
	 * 返回一个methodOverrides 对象 类初始化的时候已经初始化了
	 */
	...
	public void prepareMethodOverrides() throws BeanDefinitionValidationException {
		...
		//检查MethodOverride 容器是否为空
		if (hasMethodOverrides()) {
			//如果不为空则进行遍历并调用prepareMethodOverride方法
	 ...
	 *
	 * 进行MethodOverride属性设置
	 *
	 * @param mo the MethodOverride object to validate
	 * @throws BeanDefinitionValidationException in case of validation failure
	 */
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		//获取beanClass中MethodOverride相同名称的个数
		...
		if (count == 0) {
			...
			//将MethodOverride的Overloaded属性设置未重载不需要根据类型判断方法，提高性能
